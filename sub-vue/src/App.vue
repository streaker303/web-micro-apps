    ...
    <div id="app">
        ...
        <div class="btns">
            <template v-if="isInQiankun">
                <button @click="gotoSubVen">从当前子应用内跳转到`sub-ven`子应用</button>
                <button @click="openSubVue">独立打开sub-vue子应用</button>
            </template>
            <button @click="changeUsername">改变全局的用户名称</button>
            <div v-per="$getPer('2')">按钮权限</div>
            <div v-per="$getPer('11')">其他权限</div>
        </div>
        <router-view />
    </div>
</template>
...
export default {
    ...
    // setGlobalState 是在 /common/src/store/global-register.js中定义的
        ...
        gotoSubVen() {
            // history.pushState(null, "/#/micrApp/sub-ven", "/#/micrApp/sub-ven");
            this.$router.push({
                path: '/micrApp/sub-ven'
            })
        },
        changeUsername() {
